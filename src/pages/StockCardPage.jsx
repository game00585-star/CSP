...
import {ArrowDownToLine,ArrowUpFromLine,ArrowRightLeft,PackageCheck,Search} from 'lucide-react';
...
export default function StockCardPage(){
  ...
  return <>
    ...
    <div className="card"><div className="card-title"><h2>Lot คงเหลือ</h2><ExportButton rows={lotList} name="CSP-lot-balance"/></div><div className="table-wrap"><table><thead><tr><th>รหัสสินค้า</th><th>ชื่อสินค้า</th><th>จุดเก็บ</th><th>คลัง</th><th>Lot</th><th>วันที่รับ</th><th>หน่วย</th><th>รับทั้งหมด</th><th>คงเหลือ Lot</th></tr></thead><tbody>{lotList.map(lot=><tr key={lot.id}><td><b>{lot.productCode}</b><small>{lot.barcode}</small></td><td>{lot.productName}</td><td>{lot.locationName||'ไม่ระบุจุดเก็บ'}</td><td>{lot.warehouseGroup}</td><td><b>{lot.lotNo}</b></td><td>{lot.receivedDate}</td><td>{products.find(item=>item.id===lot.productId)?.unit||'—'}</td><td>{fmt(lot.quantityReceived)}</td><td><b>{fmt(lot.quantityRemaining)}</b></td></tr>)}</tbody></table>{!lotList.length&&<Empty/>}</div></div>
  </>;
}
