...
import {Boxes,PackageCheck,TriangleAlert,PackageX,Plus,Upload,Trash2,MapPinned,TableProperties,Save,PencilLine} from 'lucide-react';
...
export default function WarehousePage(){
  ...
  const {products,movements,stockCounts,activePeriod,viewingPeriod,addProduct,updateProduct,removeProduct,requestOpeningBalanceAdjustment,setToast}=useApp();
  const fileInput=useRef(null);
  const [search,setSearch]=useState(''),[status,setStatus]=useState(''),[unit,setUnit]=useState('');
  const [page,setPage]=useState(1),[size,setSize]=useState(10),[editing,setEditing]=useState(null),[view,setView]=useState(null),[preview,setPreview]=useState([]),[selected,setSelected]=useState([]),[confirmDelete,setConfirmDelete]=useState(false),[headerNames,setHeaderNames]=useState(readHeaderNames),[editingHeaders,setEditingHeaders]=useState(null),[flowEdit,setFlowEdit]=useState(null);
  ...
  const submitFlowEdit=event=>{event.preventDefault();try{requestOpeningBalanceAdjustment(flowEdit);setFlowEdit(null)}catch(error){setToast(error.message)}};
  ...
  return <div className="warehouse-page warehouse-page-expanded">
    ...
    {flowEdit&&<Modal title="แก้ไข IN / OUT · ยอดยกมา" onClose={()=>setFlowEdit(null)}><form className="opening-flow-form" onSubmit={submitFlowEdit}><div className="flow-product-summary"><small>รายการที่ต้องการแก้ไข</small><b>{flowEdit.productName}</b><span>{flowEdit.productCode} · คลัง {flowEdit.warehouseGroup}</span></div><div className="flow-current"><div><small>IN ปัจจุบัน</small><b>{fmt(flowEdit.oldIn)}</b></div><div><small>OUT ปัจจุบัน</small><b>{fmt(flowEdit.oldOut)}</b></div></div><div className="form-grid"><label>ช่องที่ต้องการแก้<select value={flowEdit.direction} onChange={event=>setFlowEdit(current=>({...current,direction:event.target.value}))}><option value="IN">IN</option><option value="OUT">OUT</option></select></label><label>รูปแบบการแก้ไข<select value={flowEdit.adjustment} onChange={event=>setFlowEdit(current=>({...current,adjustment:event.target.value}))}><option value="INCREASE">เพิ่มจำนวน</option><option value="DECREASE">ลดจำนวน</option></select></label><label>จำนวน<input type="number" min="0.0001" step="any" required value={flowEdit.quantity} onChange={event=>setFlowEdit(current=>({...current,quantity:event.target.value}))}/></label><label className="full">เหตุผลการแก้ไข<textarea required value={flowEdit.reason} onChange={event=>setFlowEdit(current=>({...current,reason:event.target.value}))} placeholder="ระบุเหตุผลให้ชัดเจน เพื่อส่งไปยังรายการแก้ไข"/></label></div><div className="opening-flow-note">รายการนี้มาจากเมนู <b>ยอดยกมา</b> และจะยังไม่เปลี่ยน Stock จนกว่าผู้มีระดับจัดการจะอนุมัติ</div><div className="modal-actions"><button type="button" className="btn ghost" onClick={()=>setFlowEdit(null)}>ยกเลิก</button><button type="submit" className="btn primary"><Save/> ส่งไปรายการแก้ไข</button></div></form></Modal>}
