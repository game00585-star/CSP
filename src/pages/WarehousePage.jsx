...
import {Boxes,PackageCheck,TriangleAlert,PackageX,Plus,Upload,Trash2,MapPinned,TableProperties,Save,PencilLine,CheckCircle2,RotateCcw} from 'lucide-react';
...
const openingBalanceColumns=['ลำดับ','รหัสสินค้า','ยอดยกมา','ชื่อสินค้า','กก./กระสอบ','จำนวนสินค้าคงเหลือ','หน่วย','จำนวนกระสอบ/ลัง','IN','OUT','กลุ่มสินค้า','จุดจัดเก็บ'];
...
export default function WarehousePage(){
  ...
  const {products,movements,stockCounts,storageLocations,openingClosures,activePeriod,viewingPeriod,addProduct,updateProduct:saveProduct,assignProductLocation,removeProduct,requestOpeningBalanceAdjustment,finalizeOpeningBalance,reopenOpeningBalance,setToast}=useApp();
  const fileInput=useRef(null);
  const [search,setSearch]=useState(''),[status,setStatus]=useState(''),[unit,setUnit]=useState('');
  const [page,setPage]=useState(1),[size,setSize]=useState(10),[editing,setEditing]=useState(null),[view,setView]=useState(null),[preview,setPreview]=useState([]),[selected,setSelected]=useState([]),[confirmDelete,setConfirmDelete]=useState(false),[confirmFinalize,setConfirmFinalize]=useState(false),[confirmReopen,setConfirmReopen]=useState(false),[finalizeDate,setFinalizeDate]=useState(()=>new Date().toISOString().slice(0,10)),[headerNames,setHeaderNames]=useState(readHeaderNames),[editingHeaders,setEditingHeaders]=useState(null),[flowEdit,setFlowEdit]=useState(null),[approvalNotice,setApprovalNotice]=useState(false),[moveSelection,setMoveSelection]=useState(null);
  const g=warehouseGroups.find(item=>item.path===group)||warehouseGroups[0];
  const updateProduct=(id,data)=>{const current=products.find(product=>product.id===id);if(data.defaultLocationId&&data.defaultLocationId!==current?.defaultLocationId){assignProductLocation(id,data.defaultLocationId);return}saveProduct(id,data)};
  ...
  const flowValue=(product,direction)=>{const snapshot=openingSnapshot(product),systemValue=movementValue(product,direction,snapshot),manualValue=Number(direction==='IN'?product.manualIn:product.manualOut)||0,importedValue=snapshot.fromCount?0:Number(String(sourceValue(product,direction)||0).replaceAll(',',''))||0;return systemValue+manualValue+importedValue};
  ...
  const openingTableValue=(product,column)=>{
    ...
    if(column==='จุดจัดเก็บ')return product.defaultLocationName||'ยังไม่ระบุจุดจัดเก็บ';
    ...
    if(column==='กลุ่มสินค้า')return sourceValue(product,'กลุ่มสินค้า')||product.productCategory||product.warehouseGroup||'—';
  ...
  const displayHeader=(column,index)=>column==='ยอดยกมา'?'ยอดยกมา':column==='จำนวนสินค้าคงเหลือ'?stockBalanceHeader:column==='จุดจัดเก็บ'?'แผนที่จุดเก็บ':headerNames[g.id]?.length===openingBalanceColumns.length?headerNames[g.id][index]||column:column;
  const exportRows=list.map((product,index)=>Object.fromEntries(openingBalanceColumns.map((column,columnIndex)=>[displayHeader(column,columnIndex),column==='ลำดับ'?index+1:openingTableValue(product,column)])));
  const locationOptions=storageLocations.filter(location=>location.warehouseGroup===g.id&&location.active);
  ...
  const finishOpening=event=>{event.preventDefault();try{finalizeOpeningBalance(g.id,list.map(product=>({productId:product.id,productCode:product.productCode,productName:product.productName,unit:product.unit,netBalance:calculatedBalance(product),locationId:product.defaultLocationId||null,locationName:product.defaultLocationName||'ยังไม่ระบุจุดจัดเก็บ',productCategory:sourceValue(product,'กลุ่มสินค้า')||product.productCategory||product.warehouseGroup||'—',remark:sourceValue(product,'Remark')||product.note||'—',productionDate:sourceValue(product,'วันผลิต')||product.productionDate||'—',expiryDate:sourceValue(product,'วันหมดอายุ')||product.expiryDate||'—',expiryRemaining:sourceValue(product,'วันหมดอายุที่เหลือ\n(UP DATE )')||product.expiryRemaining||'—',minStock:sourceValue(product,'MIN')??product.minStock??0,maxStock:sourceValue(product,'MAX')??product.maxStock??0,comment:sourceValue(product,'Comment')||product.comment||'—'})),finalizeDate);setConfirmFinalize(false)}catch(error){setToast(error.message)}};
  const reopenOpening=()=>{try{reopenOpeningBalance(completedClosure.id);setConfirmReopen(false)}catch(error){setToast(error.message)}};
  ...
  const openMoveSelection=()=>{const targetGroup=g.id,firstLocation=storageLocations.find(location=>location.warehouseGroup===targetGroup&&location.active);setMoveSelection({warehouseGroup:targetGroup,locationId:firstLocation?.id||''})};
  const confirmMoveSelection=()=>{if(!moveSelection?.locationId){setToast('กรุณาเลือกคลังและจุดจัดเก็บปลายทาง');return}selectedInList.forEach(id=>assignProductLocation(id,moveSelection.locationId));setSelected(current=>current.filter(id=>!selectedInList.includes(id)));setMoveSelection(null)};
  ...
  return <div className="warehouse-page warehouse-page-expanded">
    ...
    {confirmReopen&&<ConfirmModal title={`กลับมาแก้ไขยอดยกมา · ${g.name}`} text="ชุดข้อมูลที่ส่งไปยังรายการเคลื่อนไหวจะถูกถอนออก และหน้ายอดยกมาจะกลับมาแก้ไขได้อีกครั้ง การดำเนินการนี้จะถูกบันทึกใน Audit Log" onClose={()=>setConfirmReopen(false)} onConfirm={reopenOpening}/>}
    ...
    {selectedInList.length>0&&<button className="map-assign-floating" onClick={openMoveSelection}><MapPinned/> ย้ายสินค้าที่เลือก ({selectedInList.length})</button>}
    {moveSelection&&<Modal title={`ย้ายสินค้า ${selectedInList.length} รายการ`} onClose={()=>setMoveSelection(null)}><div className="storage-form"><label>คลังปลายทาง<select value={moveSelection.warehouseGroup} onChange={event=>{const warehouseGroup=event.target.value,firstLocation=storageLocations.find(location=>location.warehouseGroup===warehouseGroup&&location.active);setMoveSelection({warehouseGroup,locationId:firstLocation?.id||''})}}>{warehouseGroups.map(warehouse=><option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</select></label><label>แผนที่จุดเก็บ<select value={moveSelection.locationId} onChange={event=>setMoveSelection(current=>({...current,locationId:event.target.value}))}><option value="">เลือกจุดจัดเก็บ</option>{storageLocations.filter(location=>location.warehouseGroup===moveSelection.warehouseGroup&&location.active).map(location=><option key={location.id} value={location.id}>{location.name}</option>)}</select></label><div className="storage-current"><span>สินค้าที่เลือก</span><b>{selectedInList.length} รายการ</b></div></div><div className="modal-actions"><button className="btn ghost" onClick={()=>setMoveSelection(null)}>ยกเลิก</button><button className="btn primary" disabled={!moveSelection.locationId} onClick={confirmMoveSelection}><MapPinned/> ยืนยันย้ายสินค้า</button></div></Modal>}
