...
const excelColumns={
  PK:['ลำดับ','รหัสสินค้า','ยอดยกมาเดือน 7/69','ชื่อสินค้า','กก./กระสอบ','จำนวนสินค้าคงเหลือ','หน่วย','จำนวนกระสอบ/ลัง','กลุ่มสินค้า','Remark','วันผลิต','วันหมดอายุ','วันหมดอายุที่เหลือ\n(UP DATE )','IN','OUT','MIN','MAX','Comment'],
  IN:['ลำดับ','รหัสสินค้า','ยอดยกมาเดือน 7/69','ชื่อสินค้า','กก./กระสอบ/ลัง/ถุง','จำนวนสินค้าคงเหลือ','หน่วย','จำนวนกระสอบ/ลัง','กลุ่มสินค้า','Allergen/Non Allergen','Remark','sheif life','LOT วันที่รับเข้า','วันผลิต','วันหมดอายุ','วันหมดอายุที่เหลือ\n(UP DATE )','IN','OUT','MIN','MAX','Comment']
};
const generalColumns=['ลำดับ','รหัสสินค้า','ชื่อสินค้า','หน่วย','คงเหลือ','ต่ำสุด / สูงสุด','สถานะ','แผนที่จุดเก็บ'];
...
export default function WarehousePage(){
    ...
    reader.onload=result=>{
      try{
        ...
        setPreview(rows.map((row,index)=>{
          ...
          const excelRow=(excelColumns[g.id]||[]).map(header=>header==='จำนวนกระสอบ/ลัง'?'':cell(row,header)??'');
          return {_row:sourceRow,sourceFile:file.name,sourceRow,barcode,productCode,productName,unit:productUnit,packSize,currentStock,minStock,maxStock,excelRow,note:[productType,String(cell(row,'หมายเหตุ','Note','Comment')||'')].filter(Boolean).join(' · '),existingId:existing?.id,errors,_valid:errors.length===0};
        }));
      }catch{
  ...
  return <div className="warehouse-page warehouse-page-expanded">
    <PageHeader title={`คลังสินค้า · ${g.name}`} subtitle="ตรวจสอบยอดคงเหลือและสถานะสินค้า" actions={<><button className="btn ghost" onClick={openHeaderEditor}><TableProperties/> แก้ไขชื่อหัวตาราง</button><button className="btn ghost" onClick={()=>nav(`/warehouse-map/${g.path}`)}><MapPinned/> แผนที่จุดเก็บ</button><input ref={fileInput} hidden type="file" accept=".xlsx,.xls" onChange={readExcel}/><button className="btn secondary" onClick={()=>fileInput.current?.click()}><Upload/> Import Excel</button><button className="btn primary" onClick={()=>setEditing({warehouseGroup:g.id,excelRow:sourceColumns?.map(()=>''),unit:'กิโลกรัม',currentStock:0,minStock:0,maxStock:100})}><Plus/> เพิ่มสินค้า</button></>}/>
