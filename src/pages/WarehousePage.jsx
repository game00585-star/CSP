...
import {Boxes,PackageCheck,TriangleAlert,PackageX,Plus,Upload,Trash2,MapPinned,Save,PencilLine,CheckCircle2,RotateCcw} from 'lucide-react';
...
const openingBalanceColumns=['ลำดับ','รหัสสินค้า','ยอดยกมา','ชื่อสินค้า','กก./กระสอบ','จำนวนสินค้าคงเหลือ','หน่วย','จำนวนกระสอบ/ลัง','IN','OUT','กลุ่มสินค้า','แผนที่จุดเก็บ'];
...
export default function WarehousePage(){
  ...
  const [page,setPage]=useState(1),[size,setSize]=useState(10),[editing,setEditing]=useState(null),[view,setView]=useState(null),[preview,setPreview]=useState([]),[selected,setSelected]=useState([]),[confirmDelete,setConfirmDelete]=useState(false),[confirmFinalize,setConfirmFinalize]=useState(false),[confirmReopen,setConfirmReopen]=useState(false),[finalizeDate,setFinalizeDate]=useState(()=>new Date().toISOString().slice(0,10)),[headerNames,setHeaderNames]=useState(readHeaderNames),[editingHeaders,setEditingHeaders]=useState(null),[flowEdit,setFlowEdit]=useState(null),[approvalNotice,setApprovalNotice]=useState(false);
  ...
  const openingTableValue=(product,column)=>{
    ...
    if(column==='แผนที่จุดเก็บ')return product.defaultLocationName||'ยังไม่ระบุจุดจัดเก็บ';
  ...
  const displayHeader=(column,index)=>column==='ยอดยกมา'?'ยอดยกมา':column==='จำนวนสินค้าคงเหลือ'?stockBalanceHeader:column==='แผนที่จุดเก็บ'?'แผนที่จุดเก็บ':headerNames[g.id]?.length===openingBalanceColumns.length?headerNames[g.id][index]||column:column;
  ...
  const stats={stock:list.reduce((sum,p)=>sum+Number(p.currentStock||0),0),low:list.filter(p=>stockStatus(p)==='ใกล้หมด').length,out:list.filter(p=>stockStatus(p)==='หมด').length};
  ...
  return <div className="warehouse-page warehouse-page-expanded">
    <PageHeader title={`ยอดยกมา · ${g.name}`} subtitle="ตรวจสอบยอดยกมา ยอดคงเหลือ และสถานะสินค้า" actions={<><button className="btn ghost" onClick={()=>nav(`/warehouse-map/${g.path}`)}><MapPinned/> แผนที่จุดเก็บ</button><input ref={fileInput} hidden type="file" accept=".xlsx,.xls" onChange={readExcel}/><button className="btn secondary" disabled={Boolean(completedClosure)} onClick={()=>fileInput.current?.click()}><Upload/> Import Excel</button><button className="btn primary" disabled={Boolean(completedClosure)} onClick={()=>setEditing({warehouseGroup:g.id,excelRow:sourceColumns?.map(()=>''),unit:'กิโลกรัม',openingBalance:0,currentStock:0,minStock:0,maxStock:100})}><Plus/> เพิ่มสินค้า</button>{completedClosure?<button className="btn reopen-opening" onClick={()=>setConfirmReopen(true)}><RotateCcw/> กลับมาแก้ไข</button>:<button className="btn finish-opening" onClick={()=>setConfirmFinalize(true)}><CheckCircle2/> จบยอดยกมา</button>}</>}/>
    <div className="stats-grid compact"><StatCard icon={Boxes} label="จำนวนรายการ" value={list.length} unit="สินค้า" onClick={()=>{setStatus('');setPage(1)}}/><StatCard icon={PackageCheck} label="Stock คงเหลือ" value={fmt(stats.stock)} unit="หน่วย" color="green" onClick={()=>nav(`/stock-card?group=${g.id}`)}/><StatCard icon={TriangleAlert} label="ใกล้หมด" value={stats.low} unit="รายการ" color="orange" onClick={()=>{setStatus('ใกล้หมด');setPage(1)}}/><StatCard icon={PackageX} label="หมด" value={stats.out} unit="รายการ" color="red" onClick={()=>{setStatus('หมด');setPage(1)}}/></div>
    <div className="card"><Toolbar search={search} setSearch={setSearch}><select value={status} onChange={e=>setStatus(e.target.value)}><option value="">ทุกสถานะ</option><option>ปกติ</option><option>ใกล้หมด</option><option>หมด</option></select><select value={unit} onChange={e=>setUnit(e.target.value)}><option value="">ทุกหน่วย</option>{units.map(item=><option key={item}>{item}</option>)}</select><button className="btn ghost" onClick={()=>{setSearch('');setStatus('');setUnit('')}}>ล้างตัวกรอง</button>{selectedInList.length>0&&<button className="btn danger-btn" onClick={()=>setConfirmDelete(true)}><Trash2/> ลบที่เลือก ({selectedInList.length})</button>}<ExportButton rows={exportRows} name={`CSP-opening-balance-${g.path}`}/></Toolbar>
      <div className="table-wrap opening-balance-table"><table><thead><tr><th><input type="checkbox" checked={allSelected} onChange={toggleAll} aria-label={`เลือกสินค้าทั้งหมดในคลัง ${g.name}`}/></th>{openingBalanceColumns.map((column,index)=><th key={column}>{displayHeader(column,index)}</th>)}</tr></thead><tbody>{list.slice((page-1)*size,page*size).map((p,index)=><tr key={p.id} className={selected.includes(p.id)?'selected-row selectable-row':''} onClick={()=>toggleOne(p.id)}><td><input type="checkbox" checked={selected.includes(p.id)} onClick={event=>event.stopPropagation()} onChange={()=>toggleOne(p.id)} aria-label={`เลือก ${p.productName}`}/></td>{openingBalanceColumns.map(column=><td key={column} className={['ยอดยกมา','กก./กระสอบ','จำนวนสินค้าคงเหลือ','จำนวนกระสอบ/ลัง'].includes(column)?'num':''}>{column==='ลำดับ'?(page-1)*size+index+1:column==='จุดจัดเก็บ'?<select className="opening-location-select" value={p.defaultLocationId||''} disabled={Boolean(completedClosure)} onClick={event=>event.stopPropagation()} onChange={event=>{const location=storageLocations.find(item=>item.id===event.target.value);updateProduct(p.id,{...p,defaultLocationId:location?.id||'',defaultLocationName:location?.name||''})}}><option value="">ยังไม่ระบุจุดจัดเก็บ</option>{locationOptions.map(location=><option key={location.id} value={location.id}>{location.name}</option>)}</select>:openingTableValue(p,column)}</td>)}</tr>)}</tbody></table>{!list.length&&<Empty/>}</div>
    ...
    {selectedInList.length>0&&<div className="selection-floating-actions">{selectedInList.length===1&&<button className="btn secondary" onClick={()=>setEditing({...products.find(product=>product.id===selectedInList[0])})}><PencilLine/> แก้ไขรายการที่เลือก</button>}<button className="btn primary" onClick={openMapAssignment}><MapPinned/> นำสินค้าที่เลือก ({selectedInList.length}) ไปจัดลงแผนที่</button></div>}
