...
const computedColumns=new Set(['ลำดับ','จำนวนกระสอบ/ลัง','จำนวนสินค้าคงเหลือ']);

export default function ProductForm({product,columns,sourceColumns,columnLabels,onClose}){
  const {products,addProduct,updateProduct}=useApp(),editing=Boolean(product.id);
  const [f,setF]=useState({barcode:'',productCode:'',productName:'',warehouseGroup:'RM',unit:'กิโลกรัม',openingBalance:'',currentStock:0,minStock:0,maxStock:100,note:'',excelRow:columns?.map(()=>''),...product}),[error,setError]=useState('');
  ...
  const columnValue=(column,index)=>{
    ...
    if(column==='ยอดยกมา')return f.openingBalance??f.currentStock;
    if(column==='จำนวนสินค้าคงเหลือ')return f.currentStock;
    if(column==='หน่วย')return f.unit;
    if(column==='IN')return f.manualIn??0;
    if(column==='OUT')return f.manualOut??0;
    if(column==='กลุ่มสินค้า')return f.productCategory??'';
    if(column==='MIN')return f.minStock;
    if(column==='MAX')return f.maxStock;
    const sourceIndex=sourceColumns?.indexOf(column)??index;
    return f.excelRow?.[sourceIndex]??'';
  };
  const changeColumn=(column,index,value)=>setF(current=>{
    const excelRow=[...(current.excelRow||(sourceColumns||columns).map(()=>''))],sourceIndex=sourceColumns?.indexOf(column)??index;if(sourceIndex>=0)excelRow[sourceIndex]=value;
    const next={...current,excelRow};
    if(column==='รหัสสินค้า')next.productCode=value;
    if(column==='ชื่อสินค้า')next.productName=value;
    if(column==='ยอดยกมา')next.openingBalance=numberFrom(value);
    if(column==='จำนวนสินค้าคงเหลือ')next.currentStock=numberFrom(value);
    if(column==='หน่วย')next.unit=value;
    if(column==='IN')next.manualIn=numberFrom(value);
    if(column==='OUT')next.manualOut=numberFrom(value);
    if(column==='กลุ่มสินค้า')next.productCategory=value;
