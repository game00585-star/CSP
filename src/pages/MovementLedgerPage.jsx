...
import {Empty,ExportButton,Modal,PageHeader,Toolbar} from '../components/common';
...
export default function MovementLedgerPage(){
  const {group}=useParams(),nav=useNavigate(),{openingClosures,movements,products,lots,storageLocations,assignProductLocation,activePeriod,viewingPeriod}=useApp();
  const [search,setSearch]=useState(''),[selected,setSelected]=useState([]),[moveSelection,setMoveSelection]=useState(null);
  ...
  const openMoveSelection=()=>{const warehouseGroup=warehouse.id,firstLocation=storageLocations.find(location=>location.warehouseGroup===warehouseGroup&&location.active);setMoveSelection({warehouseGroup,locationId:firstLocation?.id||''})};
  const confirmMoveSelection=()=>{if(!moveSelection?.locationId)return;selectedRows.forEach(id=>assignProductLocation(id,moveSelection.locationId));setSelected([]);setMoveSelection(null)};
