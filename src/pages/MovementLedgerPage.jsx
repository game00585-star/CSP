...
import {Empty,ExportButton,PageHeader,Pagination,StatCard,StatusBadge,Toolbar} from '../components/common';
...
export default function MovementLedgerPage(){
  ...
  const [search,setSearch]=useState(''),[status,setStatus]=useState(''),[unit,setUnit]=useState(''),[page,setPage]=useState(1),[size,setSize]=useState(10),[selected,setSelected]=useState([]),[selectedDay,setSelectedDay]=useState('');
...
function MovementByDay({movements,warehouseGroup,month,selectedDay,setSelectedDay}){
  ...
  const groups=shown.reduce((map,item)=>{const date=item.transactionDate||'ไม่ระบุวันที่';if(!map.has(date))map.set(date,[]);map.get(date).push(item);return map},new Map());
  return <section className="card movement-day-panel"><div className="card-title"><div><h2>Movement ตามวันและเวลาจริง</h2><p>เลือกวันเพื่อดูรายการรับ จ่าย และโอนที่เกิดจากหน้าเว็บ CSP</p></div><span className="movement-result-count">{shown.length} รายการ</span></div><div className="movement-day-selector"><button className={!selectedDay?'active':''} onClick={()=>setSelectedDay('')}>ทั้งหมด <b>{monthMovements.length}</b></button>{Array.from({length:daysInMonth},(_,index)=>index+1).map(day=><button key={day} className={`${Number(selectedDay)===day?'active':''} ${counts.has(day)?'':'empty-day'}`} onClick={()=>setSelectedDay(String(day))}>{day}{counts.has(day)&&<b>{counts.get(day)}</b>}</button>)}</div><div className="movement-day-groups">{[...groups.entries()].map(([date,items])=><div className="movement-day-group" key={date}><h3>{thaiDate(date)} <span>{items.length} รายการ</span></h3>{items.map(item=><div className="movement-day-row" key={item.id}><time>{item.transactionTime||movementDateTime(item)?.toLocaleTimeString('th-TH')||'—'}</time><span className={`movement-type ${String(item.transactionType).toLowerCase()}`}>{item.transactionType}</span><b>{item.productCode} · {item.productName}</b><strong className={Number(item.quantityIn)>0?'qty-in':'qty-out'}>{Number(item.quantityIn)>0?'+':'-'}{fmt(Number(item.quantityIn||item.quantityOut||0))} {item.unit}</strong><small>{item.documentNo} · {item.userName||'—'}</small></div>)}</div>)}{!shown.length&&<Empty/>}</div></section>;
}
