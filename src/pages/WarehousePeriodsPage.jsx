...
import {CalendarDays,CheckCircle2,LockKeyhole,Plus,Trash2,TriangleAlert} from 'lucide-react';
...
export default function WarehousePeriodsPage(){
  const {warehousePeriods,activePeriod,createWarehousePeriod,closeWarehousePeriod,removeWarehousePeriod,setToast}=useApp();
  ...
  return <>
    ...
    <section className="card period-history"><div className="card-title"><h2>ประวัติรอบเดือน</h2></div><div className="table-wrap"><table><thead><tr><th>เดือน</th><th>สถานะ</th><th>เปิดโดย</th><th>วันที่เปิด</th><th>ปิดโดย</th><th>วันที่ปิด</th><th>จัดการ</th></tr></thead><tbody>{warehousePeriods.map(period=><tr key={period.id}><td><b>{monthLabel(period.month)}</b><small>{period.month}</small></td><td><span className={`period-status ${period.status.toLowerCase()}`}>{period.status==='OPEN'?'เปิดใช้งาน':'ปิดรอบแล้ว'}</span></td><td>{period.openedBy}</td><td>{new Date(period.openedAt).toLocaleString('th-TH')}</td><td>{period.closedBy||'—'}</td><td>{period.closedAt?new Date(period.closedAt).toLocaleString('th-TH'):'—'}</td><td><button className="icon-btn danger" disabled={period.status==='OPEN'} onClick={()=>setDeleting(period)} title={period.status==='OPEN'?'ต้องปิดรอบก่อนลบ':'ลบรอบเดือน'}><Trash2/></button></td></tr>)}</tbody></table>{!warehousePeriods.length&&<Empty/>}</div></section>
