...
export default function WarehousePeriodsPage(){
  ...
  return <>
    ...
    {closing&&<ConfirmModal title="ยืนยันปิดรอบเดือน" text={`ระบบจะตรวจว่านับสินค้าครบ คำนวณและ Freeze Closing ${monthLabel(closing.month)} จากนั้นสร้าง Opening Stock และเปิดเดือนถัดไปอัตโนมัติ หากข้อมูลไม่ครบระบบจะไม่ปิดรอบ`} onClose={()=>setClosing(null)} onConfirm={()=>{try{closeWarehousePeriod(closing.id);setClosing(null)}catch(error){setToast(error.message)}}}/>} 
