...
import {Warehouse,Scale,Tags,TableProperties,Hash,DatabaseBackup,RotateCcw,Save} from 'lucide-react';
...
export default function SettingsPage(){
  const [tab,setTab]=useState(0),[confirm,setConfirm]=useState(false);
  const {reset,setToast}=useApp();
  return <><PageHeader title="ตั้งค่าระบบ" subtitle="กำหนดค่าพื้นฐานและจัดการข้อมูลระบบ"/><div className="settings-layout"><div className="settings-tabs">{tabs.map(([name,Icon],index)=><button className={tab===index?'active':''} key={name} onClick={()=>setTab(index)}><Icon/>{name}</button>)}</div><div className="card settings-content"><h2>{tabs[tab][0]}</h2><p>จัดการค่าที่ใช้ร่วมกันภายในระบบ</p>
    ...
    {tab===5&&<div className="backup-box"><DatabaseBackup/><h3>ข้อมูล Mock ในอุปกรณ์นี้</h3><p>ข้อมูลสินค้า จุดเก็บ และ Stock Card บันทึกอยู่ใน localStorage ของเบราว์เซอร์</p><button className="btn danger-btn" onClick={()=>setConfirm(true)}><RotateCcw/> คืนค่า Mock Data</button></div>}
